...
import com.halitpractice.tvlangsungturkilight.adapters.YtbExtraTvLanguageAdapter;
import com.halitpractice.tvlangsungturkilight.models.YtbExtraTvLanguageModel;
import com.halitpractice.tvlangsungturkilight.services.YtbExtraTvLanguageDataCache;
...
public class YtbExtraTvLanguageActivity extends AppCompatActivity {

    private RecyclerView recyclerView;
    private List<YtbExtraTvLanguageModel> main_list;
    private YtbExtraTvLanguageAdapter adapter;
    private ProgressBar progressBar;

    private YtbExtraTvLanguageDataCache dataCache;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_ytb_extra_tv_language);
        ...
        homeButton.setOnClickListener(v -> {
            ...
            Intent homeIntent = new Intent(YtbExtraTvLanguageActivity.this, MainActivity.class);
            startActivity(homeIntent);
            finish(); // Close the current activity
        });

        progressBar = findViewById(R.id.ytbExtraTvLanguageProgressBar); // Initialize the ProgressBar
        progressBar.setVisibility(View.GONE); // Initially, set it to GONE

        main_list = new ArrayList<>();
        recyclerView = findViewById(R.id.ytbExtraTvLanguageRecycler);
        recyclerView.setHasFixedSize(true);
        recyclerView.setLayoutManager(new LinearLayoutManager(this));

        AdView mAdView = findViewById(R.id.adViewYtbExtraTvLanguage);
        AdRequest adRequest = new AdRequest.Builder().build();
        mAdView.loadAd(adRequest);
        /*
        ImageView closedBtn = findViewById(R.id.closeBtnYtbExtraTvLanguage);
...
//        fetchData();
        ...
        dataCache = YtbExtraTvLanguageDataCache.getInstance();

        // Check if there is cached data
        List<YtbExtraTvLanguageModel> cachedData = dataCache.getCachedData();
    ...
    private void fetchData() {
        ...
        List<YtbExtraTvLanguageModel> cachedData = YtbExtraTvLanguageDataCache.getInstance().getCachedData();
        ...
        if (cachedData != null && !cachedData.isEmpty()) {
            ...
            Call<List<YtbExtraTvLanguageModel>> req = ManagerAll.getInstance().ytbExtraTvLanguageFetch();
            req.enqueue(new Callback<List<YtbExtraTvLanguageModel>>() {
                ...
                public void onResponse(Call<List<YtbExtraTvLanguageModel>> call, Response<List<YtbExtraTvLanguageModel>> response) {
                    ...
                    if (response.isSuccessful()) {
                        List<YtbExtraTvLanguageModel> data = response.body();
                        // If data is fetched, cache it and update the UI
                        if (data != null && !data.isEmpty()) {
                            YtbExtraTvLanguageDataCache.getInstance().setCachedData(data);
    ...
    private void updateUIWithCachedData(List<YtbExtraTvLanguageModel> cachedData) {
        if (cachedData != null && !cachedData.isEmpty()) {
            adapter = new YtbExtraTvLanguageAdapter(cachedData, YtbExtraTvLanguageActivity.this);
...
//        Toast.makeText(this, "İnternet bağlantısı yok. Lütfen ağ ayarlarınızı kontrol edin.", Toast.LENGTH_LONG).show();
    ...
    private void redirectYonlendir() {
        ...
        Intent intent = new Intent(YtbExtraTvLanguageActivity.this, YtbExtraTvYonlendirLanguageActivity.class);
