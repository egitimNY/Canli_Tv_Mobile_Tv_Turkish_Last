...
import com.halitpractice.tvlangsungturkilight.adapters.DunyaTvLanguageAdapter;
import com.halitpractice.tvlangsungturkilight.models.DunyaTvLanguageModel;
import com.halitpractice.tvlangsungturkilight.services.WorldTvLanguageDataCache;
...
public class DunyaTvLanguageActivity extends AppCompatActivity {

    private RecyclerView recyclerView;
    private List<DunyaTvLanguageModel> main_list;
    private DunyaTvLanguageAdapter adapter;
    private ProgressBar progressBar;

    private WorldTvLanguageDataCache dataCache;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_dunya_tv_language);
        ...
        if (actionBar != null) {
            actionBar.setTitle("Dünya TV'leri DİL");
        }

        ImageView homeButton = findViewById(R.id.homeButton);
        homeButton.setOnClickListener(v -> {
            // Handle the home button click here
            Intent homeIntent = new Intent(DunyaTvLanguageActivity.this, MainActivity.class);
            startActivity(homeIntent);
            finish(); // Close the current activity
        });

        progressBar = findViewById(R.id.progressBarWorldTvLanguage); // Initialize the ProgressBar
        progressBar.setVisibility(View.GONE); // Initially, set it to GONE

        main_list = new ArrayList<>();
        recyclerView = findViewById(R.id.worldTvLanguageRecyclerView);
        recyclerView.setHasFixedSize(true);
        recyclerView.setLayoutManager(new LinearLayoutManager(this));

        AdView mAdView = findViewById(R.id.adViewWorldTvLanguage);
        AdRequest adRequest = new AdRequest.Builder().build();
        mAdView.loadAd(adRequest);
        /*
        ImageView closedBtn = findViewById(R.id.closeBtnWorldTvLanguage);
...
//        fetchData();
        ...
        dataCache = WorldTvLanguageDataCache.getInstance();

        // Check if there is cached data
        List<DunyaTvLanguageModel> cachedData = dataCache.getCachedData();
    ...
    private void fetchData() {
        ...
        List<DunyaTvLanguageModel> cachedData = WorldTvLanguageDataCache.getInstance().getCachedData();
        ...
        if (cachedData != null && !cachedData.isEmpty()) {
            ...
            Call<List<DunyaTvLanguageModel>> req = ManagerAll.getInstance().cizgiFilmTvLanguageFetch();
            req.enqueue(new Callback<List<DunyaTvLanguageModel>>() {
                @Override
                public void onResponse(Call<List<DunyaTvLanguageModel>> call, Response<List<DunyaTvLanguageModel>> response) {
                    progressBar.setVisibility(View.GONE); // Always hide the ProgressBar

                    if (response.isSuccessful()) {
                        List<DunyaTvLanguageModel> data = response.body();
                        // If data is fetched, cache it and update the UI
                        if (data != null && !data.isEmpty()) {
                            WorldTvLanguageDataCache.getInstance().setCachedData(data);
                ...
                @Override
                public void onFailure(Call<List<DunyaTvLanguageModel>> call, Throwable t) {
                    progressBar.setVisibility(View.GONE); // Always hide the ProgressBar
    ...
    private void updateUIWithCachedData(List<DunyaTvLanguageModel> cachedData) {
        if (cachedData != null && !cachedData.isEmpty()) {
            adapter = new DunyaTvLanguageAdapter(cachedData, DunyaTvLanguageActivity.this);
...
//        Toast.makeText(this, "İnternet bağlantısı yok. Lütfen ağ ayarlarınızı kontrol edin.", Toast.LENGTH_LONG).show();
    ...
    private void redirectYonlendir() {
        ...
        Intent intent = new Intent(DunyaTvLanguageActivity.this, DunyaTvYonlendirLanguageActivity.class);
