...
public class GazetelerRegionalDetailsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_gazeteler_regional_details);
        ...
        progressBar = findViewById(R.id.progressBarGazetelerRegionalDetails); // Initialize the ProgressBar
        progressBar.setVisibility(View.GONE); // Initially, set it to GONE

        ImageView homeButton = findViewById(R.id.homeButton);
        homeButton.setOnClickListener(v -> {
            // Handle the home button click here
            Intent homeIntent = new Intent(GazetelerRegionalDetailsActivity.this, MainActivity.class);
            startActivity(homeIntent);
            finish(); // Close the current activity
        });

        recyclerView = findViewById(R.id.gazetelerRegionalDetailsRecyclerView); // Initialize the RecyclerView
        recyclerView.setHasFixedSize(true);
        recyclerView.setLayoutManager(new LinearLayoutManager(this));

        String selectedCategory = getIntent().getStringExtra("regional");
        ActionBar actionBar = getSupportActionBar();
        if (actionBar != null) {
            if (selectedCategory != null) {
                actionBar.setTitle("Bölge: " + selectedCategory);
                fetchData(selectedCategory);
            } else {
                actionBar.setTitle("Bölge: ");
            }
        }

        AdView mAdView = findViewById(R.id.adViewYtbGazetelerRegionalDetails);
        AdRequest adRequest = new AdRequest.Builder().build();
        mAdView.loadAd(adRequest);

        /*
        ImageView closedBtn = findViewById(R.id.closeBtnGazetelerRegionalDetails);
    ...
    private void fetchData(String selectedCategory) {
        ...
        Call<List<GazetelerModel>> req = ManagerAll.getInstance().gazetelerRegionalDetailsFetch(selectedCategory);
            ...
            public void onResponse(Call<List<GazetelerModel>> call, Response<List<GazetelerModel>> response) {
                ...
                if (response.isSuccessful()) {
                    ...
                    if (main_list != null && !main_list.isEmpty()) {
                        adapter = new GazetelerAdapter(main_list, GazetelerRegionalDetailsActivity.this);
...
//            Toast.makeText(this, "There is no internet connection. Please check your network settings.", Toast.LENGTH_LONG).show();
    ...
    private void redirectYonlendir() {
        ...
        Intent intent = new Intent(GazetelerRegionalDetailsActivity.this, MainActivity.class);
    ...
    public void onBackPressed() {
        Intent intent = new Intent(this, GazetelerRegionalActivity.class);
        startActivity(intent);
        finish(); // Close the current activity
