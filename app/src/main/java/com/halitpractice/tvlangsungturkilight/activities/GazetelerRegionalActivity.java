...
import com.halitpractice.tvlangsungturkilight.adapters.GazetelerRegionalAdapter;
...
public class GazetelerRegionalActivity extends AppCompatActivity {

    private RecyclerView recyclerView;
    private List<GazetelerCategoryModel> main_list;
    private GazetelerRegionalAdapter adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_gazeteler_regional);
        ...
        if (actionBar != null) {
            actionBar.setTitle("Bölgesel Gazeteler");
        }

        progressBar = findViewById(R.id.progressBarGazetelerRegional); // Initialize the ProgressBar
        progressBar.setVisibility(View.GONE); // Initially, set it to GONE

        ImageView homeButton = findViewById(R.id.homeButton);
        homeButton.setOnClickListener(v -> {
            // Handle the home button click here
            Intent homeIntent = new Intent(GazetelerRegionalActivity.this, MainActivity.class);
            startActivity(homeIntent);
            finish(); // Close the current activity
        });

        main_list = new ArrayList<>();
        recyclerView = findViewById(R.id.gazetelerRegionalRecyclerView);
        recyclerView.setHasFixedSize(true);
        recyclerView.setLayoutManager(new LinearLayoutManager(this));

        AdView mAdView = findViewById(R.id.adViewGazetelerRegional);
        AdRequest adRequest = new AdRequest.Builder().build();
        mAdView.loadAd(adRequest);

        /*
        ImageView closedBtn = findViewById(R.id.closeBtnGazetelerRegional);
    ...
    private void fetchData() {
        ...
        Call<List<GazetelerCategoryModel>> req = ManagerAll.getInstance().gazetelerRegionalFetch();
            ...
            public void onResponse(Call<List<GazetelerCategoryModel>> call, Response<List<GazetelerCategoryModel>> response) {
                ...
                if (response.isSuccessful()) {
                    ...
                    if (main_list != null && !main_list.isEmpty()) {
                        adapter = new GazetelerRegionalAdapter(main_list, GazetelerRegionalActivity.this);
...
//            Toast.makeText(this, "There is no internet connection. Please check your network settings.", Toast.LENGTH_LONG).show();
    ...
    private void redirectYonlendir() {
        ...
        Intent intent = new Intent(GazetelerRegionalActivity.this, MainActivity.class);
