...
import com.halitpractice.tvlangsungturkilight.activities.GazetelerRegionalDetailsActivity;
...
public class GazetelerRegionalAdapter extends RecyclerView.Adapter<GazetelerRegionalAdapter.MyViewHolder> {
    List<GazetelerCategoryModel> my_list;
    Context context;

    public GazetelerRegionalAdapter(List<GazetelerCategoryModel> my_list, Context context) {
...
//            holder.name.setText(R.string.ytb_extra_category_not_found);
        ...
        holder.itemView.setOnClickListener(v -> {
            Intent i = new Intent(v.getContext(), GazetelerRegionalDetailsActivity.class);
            int clickedPosition = holder.getAdapterPosition();
            if (clickedPosition != RecyclerView.NO_POSITION) {
                String selectedCategory = my_list.get(clickedPosition).getName(); // Get the category name
                i.putExtra("regional", selectedCategory);
